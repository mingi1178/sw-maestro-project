...
class HwpxDocument {
     ...
     */
    static findClosingTagPosition(xml, startPos, openTag, closeTag) {
        let depth = 1;
        let pos = startPos;
    ...
    static async createFromBuffer(id, path, data) {
        ...
        if (extension.endsWith('.hwp')) {
            ...
            const content = await HwpxParser_1.HwpxParser.parse(zip);
  ...
  <hh:trackChangeConfig flags="0"/>
     ...
     */
    updateParagraphTextPreserveStyles(sectionIndex, elementIndex, newText) {
        const paragraph = this.findParagraphByPath(sectionIndex, elementIndex);
    ...
    deleteParagraph(sectionIndex, elementIndex) {
        const section = this._content.sections[sectionIndex];
        if (!section || elementIndex < 0 || elementIndex >= section.elements.length)
            return false;
        this.saveState();
    ...
    applyCharacterStyle(sectionIndex, elementIndex, runIndex, style) {
        const paragraph = this.findParagraphByPath(sectionIndex, elementIndex);
        if (!paragraph || !paragraph.runs[runIndex])
            return;
        this.saveState();
     ...
     * @returns true if successful, false otherwise
     */
    setHangingIndent(sectionIndex, elementIndex, indentPt) {
        ...
        const existingIdx = this._pendingHangingIndents.findIndex(p => p.sectionIndex === sectionIndex && p.elementIndex === elementIndex);
        if (existingIdx >= 0) {
     ...
     */
    getHangingIndent(sectionIndex, elementIndex) {
        const paragraph = this.findParagraphByPath(sectionIndex, elementIndex);
        if (!paragraph)
            return null;
     ...
     * @returns true if successful, false otherwise
     */
    removeHangingIndent(sectionIndex, elementIndex) {
        const paragraph = this.findParagraphByPath(sectionIndex, elementIndex);
        if (!paragraph)
            return false;
        this.saveState();
     ...
     */
    async loadCharPrCache() {
        if (this._charPrCache !== null || !this._zip)
            return;
        this._charPrCache = new Map();
     ...
     */
    async getFontSizeFromCharPrId(charPrId) {
     ...
     * @returns Font size in pt, or undefined if not found
     */
    async getParagraphFontSize(sectionIndex, elementIndex) {
        if (!this._zip)
            return undefined;
     ...
     * @returns Font size in pt, or undefined if not found
     */
    async getTableCellParagraphFontSize(sectionIndex, tableIndex, row, col, paragraphIndex) {
        if (!this._zip)
            return undefined;
        const sectionPath = `Contents/section${sectionIndex}.xml`;
        const sectionXml = await this._zip.file(sectionPath)?.async('string');
     ...
     */
    setAutoHangingIndent(sectionIndex, elementIndex, fontSize) {
        const paragraph = this.findParagraphByPath(sectionIndex, elementIndex);
        if (!paragraph)
            return 0;
        // Get paragraph text
     ...
     * @returns Calculated indent value in pt, or 0 if no marker detected
     */
    async setAutoHangingIndentAsync(sectionIndex, elementIndex, fallbackFontSize = 10) {
        const paragraph = this.findParagraphByPath(sectionIndex, elementIndex);
        if (!paragraph)
            return 0;
        // Get paragraph text
     ...
     * @returns true if successful, false otherwise
     */
    setTableCellHangingIndent(sectionIndex, tableIndex, row, col, paragraphIndex, indentPt) {
        // Validate indent value
        if (indentPt <= 0)
            return false;
        ...
        // Track for XML update during save (update existing entry if present)
        const existingIdx = this._pendingTableCellHangingIndents.findIndex(p => p.sectionIndex === sectionIndex &&
     ...
     * @returns Indent value in points, 0 if no hanging indent, null if invalid indices
     */
    getTableCellHangingIndent(sectionIndex, tableIndex, row, col, paragraphIndex) {
        const table = this.findTable(sectionIndex, tableIndex);
        if (!table)
            return null;
        const cell = table.rows[row]?.cells[col];
     ...
     * @returns true if successful, false otherwise
     */
    removeTableCellHangingIndent(sectionIndex, tableIndex, row, col, paragraphIndex) {
        const table = this.findTable(sectionIndex, tableIndex);
        if (!table)
            return false;
        const cell = table.rows[row]?.cells[col];
     ...
     * @returns Calculated indent value in pt, or 0 if no marker detected
     */
    setTableCellAutoHangingIndent(sectionIndex, tableIndex, row, col, paragraphIndex, fontSize) {
        const table = this.findTable(sectionIndex, tableIndex);
        if (!table)
            return 0;
        const cell = table.rows[row]?.cells[col];
     ...
     * @returns Calculated indent value in pt, or 0 if no marker detected
     */
    async setTableCellAutoHangingIndentAsync(sectionIndex, tableIndex, row, col, paragraphIndex, fallbackFontSize = 10) {
        const table = this.findTable(sectionIndex, tableIndex);
        if (!table)
            return 0;
        const cell = table.rows[row]?.cells[col];
        ...
        if (!text)
            return 0;
        // Try to get font size from document
        const docFontSize = await this.getTableCellParagraphFontSize(sectionIndex, tableIndex, row, col, paragraphIndex);
        const effectiveFontSize = docFontSize ?? fallbackFontSize;
        // Calculate indent using HangingIndentCalculator
        const calculator = new HangingIndentCalculator_1.HangingIndentCalculator();
        const indentPt = calculator.calculateHangingIndent(text, effectiveFontSize);
    ...
    findTable(sectionIndex, tableIndex) {
        ...
        return tables[tableIndex]?.data || null;
    }
    getTables() {
        const tables = [];
     ...
     */
    getTablesBySection(sectionIndex) {
        const tableMap = this.getTableMap();
        let localIndex = 0;
        return tableMap
     ...
     */
    getTablesSummary(startIndex, endIndex) {
        const tableMap = this.getTableMap();
     ...
     */
    findCellByLabel(labelText, direction = 'right') {
        const results = [];
     ...
     */
    resolvePathToPosition(path) {
        const parts = path.split('>').map(p => p.trim());
        if (parts.length < 2)
            return null;
        const labelText = parts[0];
        const directions = parts.slice(1).map(d => d.toLowerCase());
        // Find the label cell first
        const initialDir = directions[0] === 'left' || directions[0] === 'right' ? 'right' : 'down';
        const found = this.findCellByLabel(labelText, initialDir);
        if (found.length === 0)
            return null;
        // Use the first match
     ...
     */
    getCellContext(tableIndex, row, col, depth = 1) {
        const tableInfo = this.convertGlobalToLocalTableIndex(tableIndex);
        if (!tableInfo)
            return null;
        const table = this.findTable(tableInfo.section_index, tableInfo.local_index);
        ...
        for (let d = 1; d <= depth; d++) {
            ...
            if (upText !== undefined)
                result[`up_${d}`] = upText;
            if (downText !== undefined)
                result[`down_${d}`] = downText;
            if (leftText !== undefined)
                result[`left_${d}`] = leftText;
            if (rightText !== undefined)
                result[`right_${d}`] = rightText;
        }
        return result;
    }
    /**
     ...
     */
    batchFillTable(tableIndex, data, startRow = 0, startCol = 0) {
        const result = {
            success: 0,
            outOfBounds: [],
            updated: [],
     ...
     */
    convertGlobalToLocalTableIndex(globalTableIndex) {
    ...
    getDocumentOutline() {
        ...
        const getHeadingLevel = (text) => {
            ...
            if (/^제\s*[0-9]+\s*장/.test(text))
                return 1;
            if (/^제\s*[0-9]+\s*절/.test(text))
                return 2;
            if (/^[0-9]+\.\s+/.test(text))
                return 2;
            if (/^\([0-9]+\)\s*/.test(text))
     ...
     */
    getElementIndexForTable(tableIndex) {
     ...
     */
    findParagraphByText(searchText, sectionIndex) {
        const results = [];
        const lowerSearch = searchText.toLowerCase();
                ...
                if (element.type === 'paragraph') {
                    ...
                    if (text.toLowerCase().includes(lowerSearch)) {
                        ...
                        if (beforeElem?.type === 'paragraph') {
                            contextBefore = beforeElem.data.runs.map(r => r.text).join('').substring(0, 50);
                        }
                        else if (beforeElem?.type === 'table') {
                            contextBefore = '[Table]';
                        }
                        if (afterElem?.type === 'paragraph') {
                            contextAfter = afterElem.data.runs.map(r => r.text).join('').substring(0, 50);
                        }
     ...
     */
    getInsertContext(sectionIndex, elementIndex, contextRange = 2) {
        const section = this._content.sections[sectionIndex];
        if (!section)
            return null;
        const getElementSummary = (elem) => {
            if (elem.type === 'paragraph') {
                const text = elem.data.runs.map(r => r.text).join('');
     ...
     */
    findInsertPositionAfterHeader(headerText, searchIn = 'all') {
     ...
     */
    findTextInTableCells(searchText) {
     ...
     * @param tableIndex - Global table index
     */
    findInsertPositionAfterTable(tableIndex) {
     ...
     */
    deleteTable(sectionIndex, tableIndex) {
        const section = this._content?.sections?.[sectionIndex];
        if (!section?.elements)
            return false;
     ...
     */
    replaceTextInCell(sectionIndex, tableIndex, row, col, oldText, newText, options = {}) {
        const { caseSensitive = false, regex = false, replaceAll = true } = options;
        const section = this._content.sections[sectionIndex];
        if (!section) {
     ...
     */
    moveTable(sectionIndex, tableIndex, targetSectionIndex, targetAfterIndex) {
     ...
     */
    copyTable(sectionIndex, tableIndex, targetSectionIndex, targetAfterIndex) {
        // Validate source section
        if (!this._content.sections[sectionIndex]) {
            return { success: false, error: `Invalid source section index: ${sectionIndex}` };
        }
     ...
     */
    insertNestedTable(sectionIndex, parentTableIndex, row, col, nestedRows, nestedCols, options) {
        const section = this._content.sections[sectionIndex];
        if (!section) {
     ...
     */
    splitCell(sectionIndex, tableIndex, row, col) {
        // Validate section
        const section = this._content.sections[sectionIndex];
        if (!section) {
     ...
     */
    insertImageInCell(sectionIndex, tableIndex, row, col, imageData) {
        const table = this.findTable(sectionIndex, tableIndex);
        if (!table)
            return null;
        const cell = table.rows[row]?.cells[col];
    ...
    deleteSection(sectionIndex) {
        if (sectionIndex < 0 || sectionIndex >= this._content.sections.length)
            return false;
        if (this._content.sections.length <= 1)
    ...
    // ============================================================
    getStyles() {
        if (!this._content.styles?.styles)
            return [];
    ...
    getCharShapes() {
        if (!this._content.styles?.charShapes)
            return [];
        return Array.from(this._content.styles.charShapes.values());
    }
    getParaShapes() {
        if (!this._content.styles?.paraShapes)
            return [];
    ...
    async save() {
        if (!this._zip)
            throw new Error('Cannot save HWP files');
     ...
     */
    invalidateXmlPositions() {
        if (!this._content?.sections)
            return;
        for (const section of this._content.sections) {
            for (const element of section.elements) {
                if (element.type === 'paragraph') {
    ...
    getCachedXmlPosition(sectionIndex, elementIndex) {
        ...
        if (!element || element.type !== 'paragraph')
            return undefined;
        const para = element.data;
        if (!para._xmlPosition)
            return undefined;
        // Verify section index matches (safety check)
        if (para._xmlPosition.sectionIndex !== sectionIndex)
            return undefined;
        return {
    ...
    async removeFasooDrmTracking() {
        ...
        if (hpf.includes('Fasoo_Trace_ID')) {
            ...
            hpf = hpf.replace(/<opf:meta name="description" content="text">[^<]*<\/opf:meta>/, '<opf:meta name="description" content="text"></opf:meta>');
    ...
    async applyImageDeletesToZip() {
        ...
        const binaryIdsToDelete = new Set();
        const fullPathsToDelete = new Set(); // Keep original paths for BinData deletion
     ...
     */
    async applyTableDeletesToXml() {
        if (!this._zip)
            return;
        // Sort by tableIndex descending to avoid index shifting issues
        const sortedDeletes = [...this._pendingTableDeletes].sort((a, b) => b.tableIndex - a.tableIndex);
        // Group by section
        const deletesBySection = new Map();
     ...
     */
    async applyParagraphDeletesToXml() {
        if (!this._zip)
            return;
        // Sort by elementIndex descending to avoid index shifting issues
        const sortedDeletes = [...this._pendingParagraphDeletes].sort((a, b) => b.elementIndex - a.elementIndex);
        // Group by section
        const deletesBySection = new Map();
        for (const del of sortedDeletes) {
            if (!deletesBySection.has(del.sectionIndex)) {
     ...
     */
    async applyTableMovesToXml() {
        if (!this._zip || !this._pendingTableMoves || this._pendingTableMoves.length === 0)
            return;
     ...
     */
    findInsertPositionForElement(xml, afterIndex) {
        if (afterIndex < 0) {
     ...
     */
    async applyNestedTableInsertsToXml() {
        if (!this._zip)
            return;
        // Group inserts by section
        const insertsBySection = new Map();
     ...
     */
    async applyCellMergesToXml() {
        if (!this._zip)
            return;
        // Group merges by section, then by table
        const mergesBySection = new Map();
        ...
        for (const [sectionIndex, tablesMerges] of mergesBySection) {
            ...
            for (const tableIndex of sortedTableIndices) {
                ...
                for (const merge of tableMerges) {
                    const updatedTableXml = this.applyMergeToTable(tableXml, merge.startRow, merge.startCol, merge.endRow, merge.endCol);
                    if (updatedTableXml) {
     ...
     */
    applyMergeToTable(tableXml, startRow, startCol, endRow, endCol) {
        const colSpan = endCol - startCol + 1;
        const rowSpan = endRow - startRow + 1;
        // Find all rows
        const rows = this.findAllElementsWithDepth(tableXml, 'tr');
     ...
     */
    async applyCellSplitsToXml() {
        if (!this._zip)
            return;
        // Group splits by section
        const splitsBySection = new Map();
        ...
        for (const [sectionIndex, splits] of splitsBySection) {
            ...
            for (const split of sortedSplits) {
                ...
                const updatedTableXml = this.applySplitToTable(tableData.xml, split.row, split.col, split.originalColSpan, split.originalRowSpan);
                if (updatedTableXml) {
     ...
     */
    applySplitToTable(tableXml, masterRow, masterCol, colSpan, rowSpan) {
        // Find all rows
        const rows = this.findAllElementsWithDepth(tableXml, 'tr');
        ...
        for (let rowIdx = masterRow + rowSpan - 1; rowIdx >= masterRow; rowIdx--) {
            ...
            if (rowIdx === masterRow) {
                ...
                for (const cell of sortedCells) {
                    ...
                    if (cellPos !== -1)
                        cellSearchPos = cellPos + cell.xml.length;
     ...
     */
    generateEmptyCell(rowAddr, colAddr) {
        const paragraphId = Math.random().toString(36).substring(2, 11);
     ...
     */
    async applyTableCellUpdatesToXml() {
        if (!this._zip)
            return;
     ...
     */
    checkTagBalance(xml, elementName) {
     ...
     */
    validateTableStructure(tableXml) {
     ...
     */
    extractNestedTables(xml, prefix) {
        const tables = [];
     ...
     */
    extractTableFromMatch(xml, match) {
     ...
     */
    findTopLevelElements(sectionXml) {
        const results = [];
        ...
        const secCloseIdx = Math.max(sectionXml.lastIndexOf('</hs:sec>'), sectionXml.lastIndexOf('</hp:sec>'));
     ...
     */
    findAllElementsWithDepth(xml, elementName) {
        const elements = [];
     ...
     */
    updateTableCellsInXml(tableXml, updates) {
        let result = tableXml;
     ...
     */
    updateMultipleCellsInRow(rowXml, updates) {
        let result = rowXml;
     ...
     */
    updateCellInRow(rowXml, colIndex, newText, charShapeId) {
     ...
     */
    updateTextInCell(cellXml, newText, charShapeId) {
     ...
     */
    updateTextInCellChunked(cellXml, newText, charShapeId) {
     ...
     */
    updateTextInCellMultiline(cellXml, newText, charShapeId) {
     ...
     */
    async applyCellImageInsertsToXml() {
        if (!this._zip)
            return;
        // Group inserts by section
        const insertsBySection = new Map();
        ...
        for (const [sectionIndex, inserts] of insertsBySection) {
            ...
            for (const insert of sortedInserts) {
                ...
                let insertPosition;
                let insertMode = 'inside_paragraph';
                if (insert.afterText) {
  ...
  <hp:shapeComment>Inserted in cell by HWPX MCP</hp:shapeComment>
                ...
                const updatedTableXml = tableXml.substring(0, cellStartInTable) +
                    updatedCellContent +
     ...
     */
    replaceMultipleRunsInElement(xml, elementIndex, updates) {
        ...
        const extractAllParagraphs = (xmlStr) => {
            const results = [];
            const closeTag = '</hp:p>';
        ...
        const extractBalancedTags = (xmlStr, tagName) => {
            const results = [];
        ...
        const addShapeElements = (pattern, typeName) => {
            let match;
        ...
        if (nonEmptyOldTexts.length > 0) {
            ...
            const matchingParas = originalTopLevelParas.filter(para => nonEmptyOldTexts.some(escapedOld => para.xml.includes(escapedOld)));
            ...
            if (updateMap.has(i)) {
                ...
                if (/<hp:t\s*\/>/.test(run.xml)) {
                    ...
                    if (/<hp:run\b[^>]*>/.test(run.xml)) {
                        newRunXml = run.xml.replace(/(<hp:run\b[^>]*>)/, `$1<hp:t>${escapedNew}</hp:t>`);
                    }
                    else {
     ...
     */
    findParagraphById(xml, paragraphId, occurrence = 0) {
        if (!paragraphId)
            return undefined;
     ...
     */
    findParagraphByFuzzyMatch(xml, searchText, paragraphs) {
        if (!searchText || paragraphs.length === 0)
            return undefined;
        let bestMatch;
     ...
     */
    findTargetParagraphForUpdate(xml, sectionIndex, elementIndex, updates, paragraphId, paragraphOccurrence) {
     ...
     */
    replaceRunsInParagraphDirect(xml, target, updates) {
     ...
     */
    replaceTextInElementDirect(xml, target, oldText, newText) {
        const escapedNew = this.escapeXml(newText);
        let paragraphXml = xml.slice(target.start, target.end);
        if (!oldText || oldText === '') {
            ...
            if (/<hp:t\s*\/>/.test(paragraphXml)) {
                ...
                paragraphXml = paragraphXml.replace(/(<hp:run\b[^>]*>)/, `$1<hp:t>${escapedNew}</hp:t>`);
            }
        }
        else {
     ...
     */
    replaceTextInParagraphByIdAndText(xml, paragraphId, oldText, newText) {
        if (!paragraphId || !oldText)
            return xml;
        const escapedOld = this.escapeXml(oldText);
        const escapedNew = this.escapeXml(newText);
        ...
        while ((match = idPattern.exec(xml)) !== null) {
            ...
            if (!paragraphContent.includes(escapedOld))
                continue;
     ...
     */
    replaceTextInElementByIndex(xml, elementIndex, oldText, newText) {
        const escapedOld = this.escapeXml(oldText);
        const escapedNew = this.escapeXml(newText);
     ...
     */
    replaceTextInParagraphById(xml, paragraphId, runIndex, oldText, newText) {
        if (!paragraphId)
            return xml;
        const escapedOld = this.escapeXml(oldText);
        const escapedNew = this.escapeXml(newText);
        ...
        const newParagraphContent = paragraphContent.slice(0, targetRun.start) +
            newRunContent +
     ...
     */
    generateParagraphXml(paragraph) {
        let xml = `  <hp:p>\n`;
        for (const run of paragraph.runs) {
            xml += `    <hp:run>\n`;
    ...
    serializeCharShape(charShape) {
        ...
        if (charShape.color)
            xml += ` textColor="${charShape.color}"`;
        if (charShape.backgroundColor)
            xml += ` shadeColor="${charShape.backgroundColor}"`;
        if (charShape.useFontSpace !== undefined)
            xml += ` useFontSpace="${charShape.useFontSpace ? '1' : '0'}"`;
        if (charShape.useKerning !== undefined)
            xml += ` useKerning="${charShape.useKerning ? '1' : '0'}"`;
        if (charShape.borderFillId !== undefined)
            xml += ` borderFillIDRef="${charShape.borderFillId}"`;
        xml += `>`;
        // Font references
        if (charShape.fontRefs) {
            xml += `<hh:fontRef`;
            if (charShape.fontRefs.hangul !== undefined)
                xml += ` hangul="${charShape.fontRefs.hangul}"`;
            if (charShape.fontRefs.latin !== undefined)
                xml += ` latin="${charShape.fontRefs.latin}"`;
            if (charShape.fontRefs.hanja !== undefined)
                xml += ` hanja="${charShape.fontRefs.hanja}"`;
            if (charShape.fontRefs.japanese !== undefined)
                xml += ` japanese="${charShape.fontRefs.japanese}"`;
            if (charShape.fontRefs.other !== undefined)
                xml += ` other="${charShape.fontRefs.other}"`;
            if (charShape.fontRefs.symbol !== undefined)
                xml += ` symbol="${charShape.fontRefs.symbol}"`;
            if (charShape.fontRefs.user !== undefined)
                xml += ` user="${charShape.fontRefs.user}"`;
            xml += `/>`;
        }
        // Ratio (장평)
        if (charShape.ratio) {
            xml += `<hh:ratio`;
            if (charShape.ratio.hangul !== undefined)
                xml += ` hangul="${charShape.ratio.hangul}"`;
            if (charShape.ratio.latin !== undefined)
                xml += ` latin="${charShape.ratio.latin}"`;
            if (charShape.ratio.hanja !== undefined)
                xml += ` hanja="${charShape.ratio.hanja}"`;
            if (charShape.ratio.japanese !== undefined)
                xml += ` japanese="${charShape.ratio.japanese}"`;
            if (charShape.ratio.other !== undefined)
                xml += ` other="${charShape.ratio.other}"`;
            if (charShape.ratio.symbol !== undefined)
                xml += ` symbol="${charShape.ratio.symbol}"`;
            if (charShape.ratio.user !== undefined)
                xml += ` user="${charShape.ratio.user}"`;
            xml += `/>`;
        }
        // Spacing (자간) - CRITICAL for character spacing!
        if (charShape.charSpacing) {
            xml += `<hh:spacing`;
            if (charShape.charSpacing.hangul !== undefined)
                xml += ` hangul="${charShape.charSpacing.hangul}"`;
            if (charShape.charSpacing.latin !== undefined)
                xml += ` latin="${charShape.charSpacing.latin}"`;
            if (charShape.charSpacing.hanja !== undefined)
                xml += ` hanja="${charShape.charSpacing.hanja}"`;
            if (charShape.charSpacing.japanese !== undefined)
                xml += ` japanese="${charShape.charSpacing.japanese}"`;
            if (charShape.charSpacing.other !== undefined)
                xml += ` other="${charShape.charSpacing.other}"`;
            if (charShape.charSpacing.symbol !== undefined)
                xml += ` symbol="${charShape.charSpacing.symbol}"`;
            if (charShape.charSpacing.user !== undefined)
                xml += ` user="${charShape.charSpacing.user}"`;
            xml += `/>`;
        }
        // Relative size (상대크기)
        if (charShape.relSize) {
            xml += `<hh:relSz`;
            if (charShape.relSize.hangul !== undefined)
                xml += ` hangul="${charShape.relSize.hangul}"`;
            if (charShape.relSize.latin !== undefined)
                xml += ` latin="${charShape.relSize.latin}"`;
            if (charShape.relSize.hanja !== undefined)
                xml += ` hanja="${charShape.relSize.hanja}"`;
            if (charShape.relSize.japanese !== undefined)
                xml += ` japanese="${charShape.relSize.japanese}"`;
            if (charShape.relSize.other !== undefined)
                xml += ` other="${charShape.relSize.other}"`;
            if (charShape.relSize.symbol !== undefined)
                xml += ` symbol="${charShape.relSize.symbol}"`;
            if (charShape.relSize.user !== undefined)
                xml += ` user="${charShape.relSize.user}"`;
            xml += `/>`;
        }
        // Char offset (글자위치)
        if (charShape.charOffset) {
            xml += `<hh:offset`;
            if (charShape.charOffset.hangul !== undefined)
                xml += ` hangul="${charShape.charOffset.hangul}"`;
            if (charShape.charOffset.latin !== undefined)
                xml += ` latin="${charShape.charOffset.latin}"`;
            if (charShape.charOffset.hanja !== undefined)
                xml += ` hanja="${charShape.charOffset.hanja}"`;
            if (charShape.charOffset.japanese !== undefined)
                xml += ` japanese="${charShape.charOffset.japanese}"`;
            if (charShape.charOffset.other !== undefined)
                xml += ` other="${charShape.charOffset.other}"`;
            if (charShape.charOffset.symbol !== undefined)
                xml += ` symbol="${charShape.charOffset.symbol}"`;
            if (charShape.charOffset.user !== undefined)
                xml += ` user="${charShape.charOffset.user}"`;
            xml += `/>`;
        }
        // Bold/Italic
        if (charShape.bold)
        ...
        if (charShape.shadow && typeof charShape.shadow === 'object' && charShape.shadow.type !== 'None') {
            ...
            if (charShape.shadow.offsetX !== undefined)
                xml += ` offsetX="${Math.round(charShape.shadow.offsetX * 100)}"`;
            if (charShape.shadow.offsetY !== undefined)
                xml += ` offsetY="${Math.round(charShape.shadow.offsetY * 100)}"`;
            if (charShape.shadow.color)
                xml += ` color="${charShape.shadow.color}"`;
            xml += `/>`;
        }
        // Emboss/Engrave
        if (charShape.emboss)
            xml += `<hh:emboss/>`;
        if (charShape.engrave)
            xml += `<hh:engrave/>`;
        // SymMark (강조점)
     ...
     */
    async syncCharShapesToZip() {
        if (!this._zip || !this._content.styles?.charShapes)
            return;
        const headerPath = 'Contents/header.xml';
        ...
        for (const [id, charShape] of this._content.styles.charShapes) {
            ...
            const charShapePattern = new RegExp(`<hh:charShape[^>]*\\bid="${id}"[^>]*>[\\s\\S]*?</hh:charShape>`, 'i');
            const charPrPattern = new RegExp(`<hh:charPr[^>]*\\bid="${id}"[^>]*>[\\s\\S]*?</hh:charPr>`, 'i');
    ...
    async syncMetadataToZip() {
        ...
        if (headerXml && this._content.metadata) {
            ...
            if (meta.title) {
                headerXml = headerXml.replace(/<hh:title[^>]*>[^<]*<\/hh:title>/, `<hh:title>${this.escapeXml(meta.title)}</hh:title>`);
            }
            if (meta.creator) {
                headerXml = headerXml.replace(/<hh:creator[^>]*>[^<]*<\/hh:creator>/, `<hh:creator>${this.escapeXml(meta.creator)}</hh:creator>`);
            }
            if (meta.subject) {
                headerXml = headerXml.replace(/<hh:subject[^>]*>[^<]*<\/hh:subject>/, `<hh:subject>${this.escapeXml(meta.subject)}</hh:subject>`);
            }
            if (meta.description) {
                headerXml = headerXml.replace(/<hh:description[^>]*>[^<]*<\/hh:description>/, `<hh:description>${this.escapeXml(meta.description)}</hh:description>`);
            }
            this._zip.file(headerPath, headerXml);
        }
    }
     ...
     */
    splitTextIntoChunks(text, maxChunkSize = HwpxDocument.TEXT_CHUNK_SIZE) {
     ...
     */
    generateChunkedRuns(text, prefix, charAttr, maxChunkSize) {
     ...
     */
    async getSectionXml(sectionIndex) {
        if (!this._zip)
            return null;
        const sectionPath = `Contents/section${sectionIndex}.xml`;
        const file = this._zip.file(sectionPath);
     ...
     */
    async setSectionXml(sectionIndex, xml, validate = true) {
        if (!this._zip) {
        ...
        try {
            const updatedContent = await HwpxParser_1.HwpxParser.parse(this._zip);
     ...
     */
    async renderMermaidToImage(mermaidCode, sectionIndex, afterElementIndex, options) {
        if (!this._zip) {
        ...
        try {
            ...
            const compressed = pako_1.default.deflate(jsonString, { level: 9 });
     ...
     */
    async applyImageInsertsToZip() {
        if (!this._zip || this._pendingImageInserts.length === 0)
            return;
        for (const insert of this._pendingImageInserts) {
            // 1. Add image file to BinData/ folder
            ...
            await this.addImageToContentHpf(insert.imageId, binDataPath, insert.mimeType);
            // 3. Add hp:pic tag to section XML
            await this.addImageToSectionXml(insert.sectionIndex, insert.afterElementIndex, insert.imageId, insert.width, insert.height, insert.position, insert.headerText);
        }
    }
    /**
     ...
     */
    getImageDimensionsFromBase64(base64Data, mimeType) {
        try {
            const buffer = Buffer.from(base64Data, 'base64');
            if (mimeType === 'image/png') {
                // PNG format:
     ...
     */
    async addImageToContentHpf(imageId, binDataPath, mimeType) {
        if (!this._zip)
            return;
     ...
     */
    async addImageToSectionXml(sectionIndex, afterElementIndex, imageId, width, height, position, headerText) {
        if (!this._zip)
            return;
        const sectionPath = `Contents/section${sectionIndex}.xml`;
     ...
     */
    findInsertPositionByTextInXml(xml, searchText) {
        const normalizedSearch = searchText.toLowerCase().trim();
     ...
     * Extract text content from paragraph XML
     */
    extractTextFromParagraphXml(paraXml) {
     ...
     */
    extractTextFromCellXml(cellXml) {
     ...
     */
    generateImagePicXml(picId, instId, zOrder, binaryItemId, width, height, position) {
  ...
  <hp:shapeComment>Inserted by HWPX MCP</hp:shapeComment>
     ...
     */
    analyzeXmlContent(xml, sectionIndex) {
        const issues = [];
        const tagCounts = {};
        ...
        for (const tag of tagsToCheck) {
            ...
            if (balance !== 0) {
                issues.push({
                    type: (balance > 0 ? 'unclosed_tag' : 'orphan_close_tag'),
                    severity: 'error',
     ...
     */
    async repairXml(sectionIndex, options = {}) {
        if (!this._zip) {
     ...
     */
    fixTagImbalanceGlobal(xml, tagName) {
        const repairs = [];
        let modified = false;
        let result = xml;
     ...
     */
    async getRawSectionXml(sectionIndex) {
        if (!this._zip)
            return null;
        const sectionPath = `Contents/section${sectionIndex}.xml`;
        const file = this._zip.file(sectionPath);
     ...
     */
    async setRawSectionXml(sectionIndex, xml, validate = true) {
        if (!this._zip) {
    ...
    modifyContent(options) {
        switch (options.type) {
            ...
            case 'replace': {
                if (!options.oldText || options.newText === undefined)
                    return false;
    ...
    applyConsolidatedStyle(options) {
        ...
        switch (options.target) {
            case 'paragraph': {
                ...
                // 단락 스타일 처리
                const paraStyle = {};
                if (options.style.align)
                    paraStyle.align = options.style.align;
                if (options.style.lineSpacing)
                    paraStyle.lineSpacing = options.style.lineSpacing;
            ...
            case 'text': {
                ...
                if (options.style.bold !== undefined)
                    charStyle.bold = options.style.bold;
                if (options.style.italic !== undefined)
                    charStyle.italic = options.style.italic;
                if (options.style.fontSize !== undefined)
                    charStyle.fontSize = options.style.fontSize;
                if (options.style.fontColor !== undefined)
                    charStyle.fontColor = options.style.fontColor;
     ...
     */
    chunkDocument(chunkSize = 500, overlap = 100) {
        ...
        for (let secIdx = 0; secIdx < this._content.sections.length; secIdx++) {
            ...
            while (pos < sectionText.length) {
                ...
                const coveredElements = elementPositions.filter(e => (e.start >= pos && e.start < endPos) || (e.end > pos && e.end <= endPos) || (e.start <= pos && e.end >= endPos));
                ...
                if (chunkText.length < 100 && !hasTable) {
                    ...
                    if (firstLine && firstLine.length < 50) {
                        ...
                        if (/^[1-9][.)]?\s/.test(firstLine))
                            headingLevel = 1;
                        else if (/^[가-힣][.)]?\s/.test(firstLine))
                            headingLevel = 2;
                        else if (/^[①-⑩]/.test(firstLine) || /^\([1-9]\)/.test(firstLine))
                            headingLevel = 3;
                    }
                }
                const chunk = {
                ...
                if (pos >= sectionText.length - overlap)
                    break;
            }
     ...
     */
    searchChunks(query, topK = 5, minScore = 0.1) {
        ...
        if (N === 0)
            return []; // Guard against empty document
     ...
     */
    searchPositionIndex(query, type) {
     ...
     */
    getChunkAtOffset(offset) {
        if (this._documentChunks.length === 0) {
            this.chunkDocument();
        }
        return this._documentChunks.find(chunk => offset >= chunk.startOffset && offset < chunk.endOffset) || null;
    }
    /**
     ...
     */
    getChunkContext(chunkId, before = 1, after = 1) {
        if (this._documentChunks.length === 0) {
            this.chunkDocument();
        }
     ...
     */
    async applyParagraphStylesToXml() {
        if (!this._zip || this._pendingParagraphStyles.length === 0)
            return;
        ...
        if (newParaPrs.length > 0) {
            ...
            headerXml = headerXml.replace(/<hh:paraProperties\s+itemCnt="(\d+)"/, `<hh:paraProperties itemCnt="${newItemCnt}"`);
        ...
        for (const [sectionIndex, updates] of updatesBySection) {
            ...
            for (const update of sortedUpdates) {
                if (update.elementIndex < allElements.length) {
                    ...
                    const updatedContent = elem.content.replace(/paraPrIDRef="(\d+)"/, `paraPrIDRef="${update.newParaPrId}"`);
     ...
     */
    async applyCharacterStylesToXml() {
        if (!this._zip || this._pendingCharacterStyles.length === 0)
            return;
        ...
        if (newCharPrs.length > 0) {
            ...
            if (charPropsMatch) {
                ...
                headerXml = headerXml.replace(/<hh:charProperties\s+itemCnt="\d+">/, `<hh:charProperties itemCnt="${newCount}">`);
                // Insert new charPr elements before </hh:charProperties>
                headerXml = headerXml.replace('</hh:charProperties>', newCharPrs.join('\n') + '\n    </hh:charProperties>');
            }
            this._zip.file(headerPath, headerXml);
        }
    }
     ...
     */
    async applyHangingIndentsToXml() {
        if (!this._zip || this._pendingHangingIndents.length === 0)
            return;
        // Read header.xml to find existing paraPr elements and their max ID
        const headerPath = 'Contents/header.xml';
        let headerXml = await this._zip.file(headerPath)?.async('string');
        ...
        const indentToParaPrId = new Map();
        let newParaPrXml = '';
        for (const change of this._pendingHangingIndents) {
            if (change.indentPt === 0)
                continue; // Skip removal (use existing paraPr 0)
            if (indentToParaPrId.has(change.indentPt))
                continue;
            maxParaPrId++;
            const newId = maxParaPrId;
        ...
        if (newParaPrXml) {
            ...
            if (paraPropsEndMatch && paraPropsEndMatch.index !== undefined) {
                ...
                if (itemCntMatch) {
                    ...
                    headerXml = headerXml.replace(/<hh:paraProperties([^>]*)itemCnt="\d+"/, `<hh:paraProperties$1itemCnt="${newCount}"`);
                }
            }
            this._zip.file(headerPath, headerXml);
        }
        ...
        for (const [sectionIndex, changes] of changesBySection) {
            ...
            const paragraphIdToParaPrId = new Map();
            ...
            while ((pMatch = pRegex.exec(sectionXml)) !== null) {
                ...
                if (paragraphId && paragraphIdToParaPrId.has(paragraphId)) {
                    ...
                    const updatedAttrs = attrs.replace(/paraPrIDRef="\d+"/, `paraPrIDRef="${targetParaPrId}"`);
     ...
     */
    async applyTableCellHangingIndentsToXml() {
        if (!this._zip || this._pendingTableCellHangingIndents.length === 0)
            return;
        // Read header.xml to find existing paraPr elements and their max ID
        const headerPath = 'Contents/header.xml';
        let headerXml = await this._zip.file(headerPath)?.async('string');
     ...
     */
    findTableCellInXml(tableXml, targetRow, targetCol) {
        ...
        if (rowContentEnd <= rowContentStart)
            return null;
    ...
    async applyTableRowInsertsToXml() {
        ...
        for (const [sectionIndex, inserts] of bySection) {
            ...
            for (const insert of sorted) {
                const tables = this.findAllTables(xml);
                if (insert.tableIndex >= tables.length)
                    continue;
    ...
    async applyTableRowDeletesToXml() {
        if (!this._zip)
            return;
        const bySection = new Map();
    ...
    // ============================================================
    async applyTableColumnInsertsToXml() {
        if (!this._zip)
            return;
        const bySection = new Map();
    ...
    async applyTableColumnDeletesToXml() {
        if (!this._zip)
            return;
        const bySection = new Map();
     ...
     */
    findTopLevelFullElements(sectionXml) {
        const results = [];
    ...
    // ============================================================
    async applyHeaderFooterUpdatesToXml() {
        if (!this._zip)
            return;
        ...
        for (const [sectionIndex, updates] of updatesBySection) {
            ...
            const buildParagraphXml = (text, prefix) => {
...
exports.HwpxDocument = HwpxDocument;
 ...
 */
HwpxDocument.TEXT_CHUNK_SIZE = 2000;
