...
Call get_tool_guide with: template, table, image, search, read, create`
      ...
      case 'insert_paragraph': {
        ...
        if (rawText && looksLikeMarkdown(rawText)) {
          const renderer = new MarkdownRenderer(doc, sectionIndex, afterIndex);
      ...
      case 'render_markdown': {
        ...
        const sectionIndex = (args?.section_index as number) ?? 0;
        const afterIndex = args?.after_paragraph_index as number;
        const markdownText = args?.markdown_text as string;

        if (typeof afterIndex !== 'number') return error('after_paragraph_index is required');
        if (!markdownText) return error('markdown_text is required');
