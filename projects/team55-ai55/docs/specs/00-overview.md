...
| G3: 핵심 정보 변경 | Task 중요도/마감/담당자/예상소요 변경 | Agent는 변경을 **제안만**, PM이 폼에서 직접 입력. 단, Priority Agent는 담당자 없는 `todo` / `in_progress` Task만 자동 배정하고 `task_assignments`로 추적 |
...
clamp [0, 100]
```
가중치는 `02-agent-priority-spec.md`의 기본값 표를 따르며, 변경은 PR 합의 사항.
...
- **결정적 우선순위 계산** + 미배정 active Task 담당자 배정 + AI Task 분해 제안
